...
export type ThemeBackgroundChoice =
  | 'theme-default'
  | 'starfield'
  | 'gradient'
  | 'pattern'
...
const buildCustomTheme = (paletteInput?: Partial<CustomThemePalette> | null): ColorTheme => {
  const palette = normalizeCustomThemePalette(paletteInput)
  const primary = palette.primary
  const secondary = palette.secondary
  const accent = palette.accent
  const background = palette.background
  const surface = palette.surface
  const surfaceAlt = palette.surfaceAlt
  const border = palette.border
  const divider = palette.divider
  const textPrimary = palette.textPrimary
  const textSecondary = palette.textSecondary
  const textMuted = palette.textMuted
  const ctaPrimaryBg = palette.ctaPrimaryBg
  const ctaPrimaryText = palette.ctaPrimaryText
  const heroBg = palette.heroBg
  const heroAccent = palette.heroAccent
  const companyName = palette.companyName || textPrimary

  return {
    id: CUSTOM_THEME_ID,
    name: palette.name || '自定义主题',
    description: palette.description || '基于自定义配色生成',
    preview: accent,
    colors: {
      primary,
      secondary,
      ...
      gradient: {
        ...
        secondary: `linear-gradient(135deg, ${accent} 0%, ${adjustLightness(accent, 1.15)} 100%)`,
        hero: `linear-gradient(135deg, ${primary} 0%, ${accent} 55%, ${secondary} 100%)`
      },
      status: {
        success: '#10B981',
        warning: '#F59E0B',
    ...
    neutral: {
      border: border || mixColors(textMuted, surface, 0.35),
      divider: divider || mixColors(textMuted, background, 0.45),
      surfaceAlt: surfaceAlt || adjustLightness(surface, 1.06)
    },
    semantic: {
      panelBg: adjustLightness(surface, 1.02),
      panelBorder: mixColors(textMuted, surface, 0.25),
      heroBg: heroBg || adjustLightness(background, 1.08),
      heroAccent: heroAccent || accent,
      ctaPrimaryBg: ctaPrimaryBg || accent,
      ctaPrimaryText: ctaPrimaryText || getContrastColor(ctaPrimaryBg || accent),
      ctaSecondaryBorder: heroAccent || accent,
      tagBg: hexToRgbaString(accent, 0.2),
      tagText: accent,
      mutedBg: adjustLightness(background, 1.08),
      dividerStrong: divider || mixColors(textMuted, background, 0.4),
      highlight: secondary
    },
    intensity: {
      borderWeight: 1.4,
    ...
    background: {
      type: 'gradient',
      colors: [
        hexToRgbaString(accent, 0.4),
        hexToRgbaString(secondary, 0.25),
        hexToRgbaString(primary, 0.2)
      ],
      angle: 135,
      blur: 120,
      overlayOpacity: 0.7
    ...
    visual: {
      series: buildSeries(primary, accent, secondary),
      grid: hexToRgbaString(textMuted, 0.18),
      tooltipBg: hexToRgbaString(surface, 0.95),
...
let customThemePalette: CustomThemePalette = { ...DEFAULT_CUSTOM_THEME_PALETTE }
let customTheme: ColorTheme | null = null

export const setCustomThemePalette = (palette?: Partial<CustomThemePalette> | null) => {
  customThemePalette = normalizeCustomThemePalette(palette)
  customTheme = buildCustomTheme(customThemePalette)
}

export const getCustomThemePalette = (): CustomThemePalette => customThemePalette

export const getCustomTheme = (): ColorTheme => customTheme || buildCustomTheme(customThemePalette)

export const createCustomTheme = (palette?: Partial<CustomThemePalette> | null): ColorTheme =>
  buildCustomTheme(palette)
...
const buildMeshEffect = (theme: ColorTheme): ThemeBackgroundEffect => ({
  type: 'mesh',
  baseColor: theme.semantic.panelBg || theme.colors.background,
  colors: [
    hexToRgbaString(theme.colors.accent, 0.5),
    hexToRgbaString(theme.colors.secondary, 0.35),
    hexToRgbaString(theme.semantic.heroAccent || theme.colors.primary, 0.3)
  ]
})

const buildGridEffect = (theme: ColorTheme): ThemeBackgroundEffect => ({
  type: 'grid',
  lineColor: hexToRgbaString(theme.colors.accent, 0.18),
  glowColor: hexToRgbaString(theme.colors.accent, 0.55),
  size: 48
})
...
export const resolveBackgroundEffect = (
  ...
  if (preference === 'mesh') {
    return buildMeshEffect(theme)
  }
  if (preference === 'grid') {
    return buildGridEffect(theme)
  }
  return theme.background
}
...
export const getCurrentThemeId = (): string => {
  ...
  if (root.classList.contains(`theme-${CUSTOM_THEME_ID}`)) {
