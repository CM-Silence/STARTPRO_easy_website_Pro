...
export default function AdminSettingsPage() {
  ...
  const {
    ...
    defaultValues: {
      ...
      page_transition: 'slide' as PageTransitionChoice,
    ...
    try {
      ...
      if (response.success) {
        ...
        const dataWithDefaults: Settings = {
          ...
          page_transition: (serverSettings.page_transition as PageTransitionChoice) || 'slide',
          transition_main_title: (serverSettings as any)?.transition_main_title || '',
          transition_subtitle: (serverSettings as any)?.transition_subtitle || '',
          smooth_scroll: (serverSettings as any)?.smooth_scroll || 'on',
          smooth_scroll_duration: (serverSettings as any)?.smooth_scroll_duration || '0.9'
        }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-semantic-panel p-6 rounded-xl shadow-xl border border-semantic-panelBorder">
            ...
            <div className="grid gap-4 md:grid-cols-2 mb-6">
              <div>
                <ThemeColorField
                  selectProps={register('site_theme')}
                  customThemeConfigured={Boolean(settings?.custom_theme)}
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-theme-text mb-1">页面过渡</label>
                <ThemeAwareSelect {...register('page_transition' as const)}>
                ...
                <p className="text-xs text-theme-textSecondary mt-1">页面跳转时的过渡动效；会与系统「减弱动态效果」互斥。</p>
              </div>
              <div className="md:col-span-2">
                <div className="grid gap-4 md:grid-cols-2 mb-2">
                  <div>
                    <label className="block text-sm font-medium text-theme-text mb-1">转场主标题</label>
                    <ThemeAwareInput
                      {...register('transition_main_title')}
                      placeholder="默认取站点名称，如 湃联智能"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-theme-text mb-1">转场副标题</label>
                    <ThemeAwareInput
                      {...register('transition_subtitle')}
                      placeholder="如 让智能算法安全驱动能源世界"
                    />
                  </div>
                </div>
                <p className="text-xs text-theme-textSecondary">转场幕布上品牌大字与其下副标题；留空则使用默认文本。</p>
              </div>
              <div className="md:col-span-2 rounded-lg border border-semantic-panelBorder p-3 bg-black/5 dark:bg-white/5">
                <label className="flex items-center justify-between mb-3 cursor-pointer">
                  <span className="text-sm font-medium text-theme-text">
                    平滑滚动
                    <span className="block text-xs font-normal text-theme-textSecondary mt-0.5">限速滚轮并让图片/文字随滚动缓慢移动</span>
                  </span>
                  <input
                    type="checkbox"
                    checked={watch('smooth_scroll') !== 'off'}
                    onChange={(e) => setValue('smooth_scroll', e.target.checked ? 'on' : 'off', { shouldDirty: true })}
                    className="w-4 h-4 accent-tech-accent"
                  />
                </label>
                <label className="block text-xs font-medium text-theme-textSecondary mb-1">
                  滚动平滑度（限速强度） {parseFloat(watch('smooth_scroll_duration') || '0.9').toFixed(1)}s
                </label>
                <input
                  type="range"
                  min={0.4}
                  max={1.2}
                  step={0.1}
                  value={parseFloat(watch('smooth_scroll_duration') || '0.9')}
                  onChange={(e) => setValue('smooth_scroll_duration', e.target.value, { shouldDirty: true })}
