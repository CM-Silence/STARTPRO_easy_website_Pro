...
const BannerCarousel: React.FC<BannerCarouselProps> = ({
  ...
  // 保持 ref 与 goToNextSlide 同步，供 interval 使用
  ...
  useEffect(() => {
    ...
    const imageUrls = slides.map(s => s.image).filter(Boolean) as string[]
  ...
  return (
    ...
    >
      {/* 幻灯片容器 */}
      <div
        className={`relative w-full ${useAutoHeight ? '' : 'h-96 md:h-[500px]'} transition-transform duration-500 ease-in-out will-change-transform`}
        style={{
