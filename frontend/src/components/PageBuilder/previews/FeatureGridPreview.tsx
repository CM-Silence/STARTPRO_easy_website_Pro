      ...
      <div className={`feature-grid-container grid grid-cols-1 md:grid-cols-2 gap-8 ${
        ...
        {features.map((feature: any, index: number) => (
          <HoverFX
            ...
            className={`feature-card ${hoverEnabled ? "group/card " : ""}relative bg-color-surface p-8 rounded-2xl shadow-lg border border-color-border overflow-hidden`}
          >
            {/* 背景装饰 */}
            <div
              className={`feature-card-decoration absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-primary/10 to-secondary/10 rounded-full -mr-10 -mt-10 ${hoverEnabled ? 'group-hover/card:scale-150 transition-transform' : ''}`}
