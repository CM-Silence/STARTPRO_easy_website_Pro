      ...
      <div className={componentClass}>
        ...
        <div className={`pricing-cards-grid grid ${gridCols} gap-8 w-full`}>
          {plans.map((plan: any, index: number) => {
            ...
            return (
              <HoverFX key={index} hover={hover} duration={grabMotionSettings(component.props).hoverDuration}>
                <motion.div
                  className={`pricing-card relative rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-200 border-2 group ${
                    recommended
                ...
                <div className="relative z-10">
                  ...
                  {planLink ? (
                    <motion.a
                      href={planLink}
                      className={`pricing-card-button w-full py-4 px-6 rounded-xl font-semibold text-lg transition-all duration-200 transform text-center inline-flex items-center justify-center ${
                        recommended
                      ...
                      transition={getHoverTransition(component.props)}
                    >
                      查看方案
                    </motion.a>
                  ) : (
                    <motion.button
                      className={`pricing-card-button w-full py-4 px-6 rounded-xl font-semibold text-lg transition-all duration-200 transform ${
                        recommended
