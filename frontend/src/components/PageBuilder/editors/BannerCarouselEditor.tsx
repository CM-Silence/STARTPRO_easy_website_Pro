    ...
    <div className="mb-6">
      ...
      {slides.map((slide: any, index: number) => (
        <div key={index} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg space-y-3 mb-3">
          ...
          {/* 控件亮暗色 */}
          <div className="space-y-2">
            <label className="block text-xs font-medium text-gray-700 dark:text-gray-300">控件亮暗色</label>
            <select
              value={slide.overlayTheme || 'light'}
              onChange={(e) => onChange(index, 'overlayTheme', e.target.value)}
              className="w-full px-2 py-1 text-sm rounded theme-input"
            >
              <option value="light">亮色（深色箭头/圆点）</option>
              <option value="dark">暗色（白色箭头/圆点）</option>
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  onChange={(e) => onChange(index, 'titleColor', e.target.value)}
                  className="h-8 w-12 rounded border border-gray-300 dark:border-gray-600 bg-transparent cursor-pointer"
                />
                <input
                  type="text"
                  key={`tc-${index}-${slide.titleColor || 'none'}`}
                  defaultValue={slide.titleColor || ''}
            ...
            <div className="space-y-1">
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300">描述颜色</label>
              <div className="flex items-center space-x-2">
                <input
                  type="color"
                  value={slide.descriptionColor || '#ffffff'}
                  onChange={(e) => onChange(index, 'descriptionColor', e.target.value)}
                  className="h-8 w-12 rounded border border-gray-300 dark:border-gray-600 bg-transparent cursor-pointer"
                />
                <input
                  type="text"
                  key={`dc-${index}-${slide.descriptionColor || 'none'}`}
                  defaultValue={slide.descriptionColor || ''}
