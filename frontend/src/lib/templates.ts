...
import { DEFAULT_MOTION } from '@/styles/motion-presets'

// 给每个组件的 defaultProps 注入统一的动效默认配置（浅拷贝），使新建组件自带 scroll-reveal。
const withMotionDefaults = <T extends ComponentDefinition>(def: T): T => ({
  ...def,
  defaultProps: {
