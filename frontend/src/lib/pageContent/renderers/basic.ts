  ...
  const output = (list || [])
    .map((banner: any) => {
      ...
      const resolvedTitleColor =
        banner.titleColor ||
        (titleColorMode === 'custom' && customTitleColor ? customTitleColor : '')
      const resolvedDescColor =
        banner.descriptionColor ||
        (subtitleColorMode === 'custom' && customSubtitleColor ? customSubtitleColor : '')
      const titleStyle = resolvedTitleColor ? ` style="color:${escapeHtml(resolvedTitleColor)}"` : ''
      const descStyle = resolvedDescColor ? ` style="color:${escapeHtml(resolvedDescColor)}"` : ''
      ...
      ${banner.description ? `<p${descStyle}>${escapeHtml(banner.description)}</p>` : ''}
    </div>`
    })
    .join('')
