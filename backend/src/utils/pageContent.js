...
const renderers = {
  ...
  'call-to-action': (c) => {
    const p = c.props || {}
    const titleColorMode = p.titleColorMode || 'default'
    const customTitleColor = p.customTitleColor || ''
    const subtitleColorMode = p.subtitleColorMode || 'default'
    const customSubtitleColor = p.customSubtitleColor || ''
    const buttonColorMode = p.buttonColorMode || 'default'
    const customButtonColor = p.customButtonColor || ''
    const titleColor = titleColorMode === 'custom' && customTitleColor ? customTitleColor : ''
    const subtitleColor = subtitleColorMode === 'custom' && customSubtitleColor ? customSubtitleColor : ''
    const buttonColor = buttonColorMode === 'custom' && customButtonColor ? customButtonColor : ''
    const titleStyle = titleColor ? ` style="color:${escapeHtml(titleColor)}"` : ''
    const subtitleStyle = subtitleColor ? ` style="color:${escapeHtml(subtitleColor)}"` : ''
    const btnStyle = buttonColor ? ` style="color:${escapeHtml(buttonColor)}"` : ''
    const btn = p.buttonText ? `<a class="cta-button" href="${escapeHtml(p.buttonLink || '#')}"${btnStyle}>${escapeHtml(p.buttonText)}</a>` : ''
    const style = p.backgroundColor ? ` style="background-color:${escapeHtml(p.backgroundColor)}"` : ''
    return wrapSection(
      'call-to-action',
      `<div class="cta-content"${style}>${p.title ? `<h2${titleStyle}>${escapeHtml(p.title)}</h2>` : ''}${p.subtitle ? `<p${subtitleStyle}>${escapeHtml(p.subtitle)}</p>` : ''}${btn}</div>`
    )
  },
  'faq-section': (c) => {
    const p = c.props || {}
  ...
  'news-list': (c) => {
    const p = c.props || {}
    const listItems = Array.isArray(p.items) && p.items.length > 0 ? p.items : (Array.isArray(p.articles) ? p.articles : [])
    const list = listItems
      .map(
        (n) => `<article class="news-item">${
          n.image
            ? `<div class="news-item-image"><img src="${escapeHtml(n.image)}" alt="${escapeHtml(n.title || '')}" loading="lazy" /></div>`
            : ''
        }${renderHeading('h3', n.title)}${
