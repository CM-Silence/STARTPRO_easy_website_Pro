...
const settingsSchemas = {
  update: Joi.object({
    ...
    theme_background: Joi.string()
      .valid('theme-default', 'starfield', 'gradient', 'pattern', 'mesh', 'grid')
      .optional(),
    page_transition: Joi.string().valid('fade', 'slide', 'none').optional(),
    transition_main_title: Joi.string().max(200).optional().allow('', null),
    transition_subtitle: Joi.string().max(200).optional().allow('', null),
    smooth_scroll: Joi.string().valid('on', 'off').optional(),
    smooth_scroll_duration: Joi.string().optional().allow(''),
    custom_theme: Joi.object({
      ...
      accent: Joi.string().max(20).required(),
      background: Joi.string().max(20).required(),
      surface: Joi.string().max(20).required(),
      surfaceAlt: Joi.string().max(20).optional().allow('', null),
      border: Joi.string().max(20).optional().allow('', null),
      divider: Joi.string().max(20).optional().allow('', null),
      textPrimary: Joi.string().max(20).required(),
      textSecondary: Joi.string().max(20).required(),
      textMuted: Joi.string().max(20).required(),
      ctaPrimaryBg: Joi.string().max(20).optional().allow('', null),
      ctaPrimaryText: Joi.string().max(20).optional().allow('', null),
      heroBg: Joi.string().max(20).optional().allow('', null),
      heroAccent: Joi.string().max(20).optional().allow('', null),
      companyName: Joi.string().max(20).optional().allow('', null)
    }).optional(),
    verification_tags: Joi.object({
      google: Joi.string().max(200).optional().allow('', null),
      bing: Joi.string().max(200).optional().allow('', null),
...
// 新闻中心校验
// date 只用字符串校验(YYYY-MM-DD)且不做 Date 强转，避免 Joi 把值变成 Date 对象后无法入库
const dateLike = () =>
...
const newsSchemas = {
  create: Joi.object({
    ...
    summary: Joi.string().allow('', null).max(5000).optional(),
    link: Joi.string().allow('', null).max(500).optional(),
    image: Joi.string().allow('', null).max(500).optional(),
...
module.exports = {
  ...
  validateCreateNews: validate(newsSchemas.create),
  validateUpdateNews: validate(newsSchemas.update)
}
